...
fun lt_option :: "nat option \<Rightarrow> nat option \<Rightarrow> bool" where
  "lt_option None _ = False" 
| "lt_option _ None = False"  
...
definition max_honest_node :: "bool list \<Rightarrow> nat" where
  "max_honest_node w = Max {r. r \<in> H w}"
...
proof -
  ...
  proof - 
    ...
    then have set_nil:"set_of_tines F = {[]}"
      by (smt Collect_cong length_greater_0_conv list.size(3) singleton_conv)
...
"\<exists> F. (isFork [] F \<and> closedFork F []  \<and> (m [] = (lambda F [], margin F [])))"
  by (metis ListOfAdverse.Nil closedFork_ListOfAdverse isFork_Nil lambda_of_nil_aux lemma_4_18_trivial_case_m margin_no_honest margin_of_nil_aux root_label_0.cases)
